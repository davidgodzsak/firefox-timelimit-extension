...
export async function updateBadgeForTab(tabId, url) {
  if (!tabId || !url) {
    console.warn('[BadgeManager] Invalid parameters for updateBadgeForTab:', { tabId, url });
    return;
  }
  ...
  try {
    ...
    if (_badgeCache.has(cacheKey)) {
      const cachedText = _badgeCache.get(cacheKey);
      await _setBadgeText(tabId, cachedText);
      return;
    }
    
    // Get site and usage data
    ...
    _badgeCache.set(cacheKey, badgeText);
    setTimeout(() => {
      _badgeCache.delete(cacheKey);
    }, 30000);
    ...
    console.error('[BadgeManager] Error updating badge for tab:', error);
...
export async function handleTabActivation(activeInfo) {
  ...
  try {
    ...
    if (_badgeUpdateTimer) {
      clearTimeout(_badgeUpdateTimer);
    }
    
    _badgeUpdateTimer = setTimeout(() => {
