...
function createActivityElement(activity) {
  ...
  div.setAttribute(
    'aria-label',
    `Suggested activity: ${activity.text}. Click to get a new suggestion.`
  );
  ...
  requestAnimationFrame(() => {
    ...
    if (validActivities.length === 0) {
      ...
      emptyDiv.textContent =
        'No alternative activities configured. Why not take a moment to step away from the screen?';
    ...
    try {
      ...
      emptyDiv.className = 'activity-empty';
      emptyDiv.textContent =
        'Unable to load alternative activities. Consider taking a break!';
      container.appendChild(emptyDiv);
    }
  });
}
  ...
  try {
    ...
    if (response && response.success) {
      ...
      if (error?.type === 'EXTENSION_CONTEXT_ERROR') {
        ...
        console.log(
          `[Timeout] Storage error, retrying in ${CONFIG.RETRY_DELAY}ms (attempt ${attempt}/${CONFIG.RETRY_ATTEMPTS})`
        );
        await new Promise((resolve) => setTimeout(resolve, CONFIG.RETRY_DELAY));
        return fetchRandomTimeoutNote(attempt + 1);
      } else if (error?.isRetryable && attempt < CONFIG.RETRY_ATTEMPTS) {
        console.log(
          `[Timeout] Retryable error, retrying in ${CONFIG.RETRY_DELAY}ms (attempt ${attempt}/${CONFIG.RETRY_ATTEMPTS})`
        );
        await new Promise((resolve) => setTimeout(resolve, CONFIG.RETRY_DELAY));
        return fetchRandomTimeoutNote(attempt + 1);
      }
    ...
    console.error(
      '[Timeout] Error communicating with background script:',
      error
    );
    ...
    if (
      ...
      console.error(
        '[Timeout] Extension context invalidated, page may need refresh'
      );
      return null;
    }
    ...
    if (attempt < CONFIG.RETRY_ATTEMPTS) {
      console.log(
        `[Timeout] Error occurred, retrying in ${CONFIG.RETRY_DELAY}ms (attempt ${attempt}/${CONFIG.RETRY_ATTEMPTS})`
      );
      await new Promise((resolve) => setTimeout(resolve, CONFIG.RETRY_DELAY));
      return fetchRandomTimeoutNote(attempt + 1);
    }
...
function handleInitializationError(error, params) {
  ...
  try {
    ...
    if (blockReasonElement) {
      blockReasonElement.textContent =
        'Your daily time limit for this site has been reached.';
    }
  }
  ...
  if (container) {
    ...
    if (
      ...
      errorMessage =
        'Extension was reloaded. Please refresh this page for full functionality.';
      ...
      errorMessage =
        'Unable to load activities due to storage error. Consider taking a break!';
    } else {
      errorMessage =
        'Unable to load alternative activities. Why not take a moment to step away from the screen?';
    }
...
async function initializeTimeoutPage() {
  try {
    ...
    const loadingTimeout = setTimeout(() => {
      console.warn(
        '[Timeout] Loading timeout reached, showing fallback message'
      );
      const container = document.getElementById('alternative-activities');
      if (container && container.classList.contains('loading')) {
        ...
        timeoutDiv.textContent =
          'Loading activities is taking longer than expected. Consider taking a break!';
...
function handleVisibilityChange() {
  if (!document.hidden) {
    ...
    if (
      ...
      console.log(
        '[Timeout] In error/empty state, attempting to reload activities'
      );
...
async function shuffleMotivationalNote() {
  ...
  try {
    ...
    const availableCachedNotes = cachedEntries
      .filter(
        (entry) => entry.note && entry.note.id !== _pageState.currentNote?.id
      )
    ...
    if (availableCachedNotes.length > 0) {
      ...
      randomNote =
        availableCachedNotes[
          Math.floor(Math.random() * availableCachedNotes.length)
        ];
...
function setupEventListeners() {
  ...
  if (activitiesContainer) {
    ...
    console.log(
      '[Timeout] Activity item click and keyboard event listeners added'
    );
  } else {
    console.warn(
      '[Timeout] Activities container not found for event delegation'
    );
  }
  ...
  _pageState.cleanupInterval = setInterval(() => {
    ...
    if (_pageState.cachedNotes.size > 0) {
      console.log(
        `[Timeout] Cache stats: ${_pageState.cachedNotes.size} entries`
      );
    }
