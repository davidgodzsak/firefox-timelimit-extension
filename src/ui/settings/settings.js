...
class SettingsManager {
  ...
  handleBroadcastUpdate(message) {
    ...
    console.log(
      '[Settings] Received broadcast update:',
      message.updateType,
      message.data
    );
    ...
    switch (message.updateType) {
      case 'quickLimitAdded':
        ...
        if (message.data.site) {
          console.log(
            '[Settings] Quick limit added from popup, refreshing sites'
          );
          this.distractingSites.push(message.data.site);
          this.renderSites();
          this.showToast(
            `Limit added for "${message.data.site.urlPattern}" from popup.`,
            'info'
          );
        }
        break;
  ...
  bindEvents() {
    ...
    this.elements.siteUrlInput.addEventListener(
      'blur',
      this.validateSiteUrl.bind(this)
    );
    this.elements.timeLimitInput.addEventListener(
      'blur',
      this.validateTimeLimit.bind(this)
    );
    this.elements.openLimitInput.addEventListener(
      'blur',
      this.validateOpenLimit.bind(this)
    ); // NEW: Open limit validation
    this.elements.noteTextInput.addEventListener(
      'blur',
      this.validateNoteText.bind(this)
    );
  ...
  async loadAllSettings() {
    try {
      ...
      console.log(
        '[Settings] Sending getAllSettings message to background script...'
      );
      ...
      if (response && response.success) {
        ...
        if (error?.type === 'EXTENSION_CONTEXT_ERROR') {
          ...
          throw new Error(
            error?.message || 'Failed to load settings - no success response'
          );
        }
      }
    } catch (error) {
      ...
      if (
        ...
        throw new Error(
          'Extension API not available. Please ensure this is running as an extension.'
        );
      } else if (error.message.includes('Storage error')) {
        throw new Error(
          'Failed to load settings from storage. Please try refreshing the page.'
        );
      } else {
  ...
  async handleAddSite() {
    ...
    if (!hasTimeLimit && !hasOpenLimit) {
      this.showToast(
        'Please specify at least one limit (time or opens).',
        'warning'
      );
      return;
    }
    ...
    if (
      this.distractingSites.some(
        (site) => site.urlPattern.toLowerCase() === urlPattern.toLowerCase()
      )
    ) {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        this.showToast(
          `Added "${urlPattern}" with ${limitDescription}.`,
          'success'
        );
      } else {
        ...
        if (error?.type === 'VALIDATION_ERROR') {
          ...
          this.showToast(
            error.message || 'Storage error occurred. Please try again.',
            'error'
          );
          ...
          this.showToast(
            error?.message || 'Failed to add site. Please try again.',
            'error'
          );
        }
      }
  ...
  async handleAddNote() {
    ...
    // Check for duplicates
    if (
      this.timeoutNotes.some(
        (note) => note.text.toLowerCase() === noteText.toLowerCase()
      )
    ) {
  ...
  async handleEditSite(siteId, updates) {
    if (!siteId || !updates || typeof updates !== 'object') {
      console.error('[Settings] Invalid parameters for handleEditSite:', {
        siteId,
        updates,
      });
      return;
    ...
    try {
      ...
      if (response && response.success) {
        // Update local data
        const siteIndex = this.distractingSites.findIndex(
          (site) => site.id === siteId
        );
        if (siteIndex !== -1) {
          this.distractingSites[siteIndex] = {
            ...this.distractingSites[siteIndex],
            ...updates,
          };
        ...
        if (updates.dailyLimitSeconds !== undefined) {
          ...
          changeDescriptions.push(
            `time limit to ${minutes > 0 ? minutes + ' minutes' : 'none'}`
          );
        }
        if (updates.dailyOpenLimit !== undefined) {
          changeDescriptions.push(
            `open limit to ${updates.dailyOpenLimit > 0 ? updates.dailyOpenLimit + ' opens' : 'none'}`
          );
        }
        if (updates.isEnabled !== undefined) {
          changeDescriptions.push(
            `status to ${updates.isEnabled ? 'enabled' : 'disabled'}`
          );
        }
  ...
  async handleDeleteSite(siteId) {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        this.distractingSites = this.distractingSites.filter(
          (site) => site.id !== siteId
        );
        this.renderSites();
        ...
        if (site) {
          this.showToast(
            `Removed "${site.urlPattern}" from your list.`,
            'success'
          );
        } else {
  ...
  async handleEditNote(noteId, newText) {
    try {
      ...
      if (response && response.success) {
        const noteIndex = this.timeoutNotes.findIndex(
          (note) => note.id === noteId
        );
        if (noteIndex !== -1) {
  ...
  validateSiteUrl() {
    const urlPattern = this.elements.siteUrlInput.value.trim();
    const errorElement =
      this.elements.siteUrlInput.parentElement.querySelector('.error-message');
    ...
    if (
      dangerousPatterns.some((pattern) =>
        urlPattern.toLowerCase().includes(pattern)
      )
    ) {
      this.showFieldError(
        this.elements.siteUrlInput,
  ...
  validateTimeLimit() {
    const timeLimit = parseInt(this.elements.timeLimitInput.value);
    const errorElement =
      this.elements.timeLimitInput.parentElement.querySelector(
        '.error-message'
      );
  ...
  validateOpenLimit() {
    ...
    const errorElement =
      this.elements.openLimitInput.parentElement.querySelector(
        '.error-message'
      );
  ...
  validateNoteText() {
    const noteText = this.elements.noteTextInput.value.trim();
    const errorElement =
      this.elements.noteTextInput.parentElement.querySelector('.error-message');
  ...
  renderSites() {
    ...
    // Render site items
    const siteItems = this.distractingSites.map((site) =>
      this.createSiteItem(site)
    );

    // Clear existing items and add new ones
    const existingItems =
      this.elements.sitesList.querySelectorAll('.item-card');
  ...
  renderNotes() {
    ...
    // Render note items
    const noteItems = this.timeoutNotes.map((note) =>
      this.createNoteItem(note)
    );

    // Clear existing items and add new ones
    const existingItems =
      this.elements.notesList.querySelectorAll('.item-card');
    existingItems.forEach((item) => item.remove());
  ...
  createNoteItem(note) {
    ...
    const editPath1 = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
    editPath1.setAttribute(
      'd',
      'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7'
    );
    const editPath2 = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'path'
    );
    editPath2.setAttribute(
      'd',
      'm18.5 2.5 a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z'
    );
  ...
  showLoading(show) {
    ...
    console.log(
      `[Settings] Loading overlay classes: ${this.elements.loadingOverlay.className}`
    );
  }
  ...
  showToast(message, type = 'info') {
    ...
    switch (type) {
      case 'success': {
        const successPolyline = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'polyline'
        );
      ...
      case 'error': {
        const errorCircle = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'circle'
        );
        ...
        const errorLine1 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
        ...
        const errorLine2 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
      ...
      case 'warning': {
        const warningPath = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'path'
        );
        warningPath.setAttribute('d', 'm21,16-10-17L1,16z');
        const warningLine1 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
        ...
        const warningLine2 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
      ...
      default: {
        ...
        const infoLine1 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
        ...
        const infoLine2 = document.createElementNS(
          'http://www.w3.org/2000/svg',
          'line'
        );
  ...
  async handleDeleteNote(noteId) {
    ...
    try {
      ...
      if (response && response.success) {
        this.timeoutNotes = this.timeoutNotes.filter(
          (note) => note.id !== noteId
        );
        this.renderNotes();
