...
jest.unstable_mockModule(
  '../../background_scripts/usage_recorder.js',
  () => mockUsageRecorder
);
jest.unstable_mockModule(
...
describe('Core Functionality Integration After Fixes', () => {
  ...
  beforeEach(async () => {
    ...
    mockUsageStorage.updateUsageStats.mockImplementation(
      async (dateKey, siteId, stats) => {
    ...
    mockDistractionDetector.initializeDistractionDetector.mockResolvedValue(
      true
    );
    mockDistractionDetector.loadDistractingSitesFromStorage.mockResolvedValue(
      true
    );
  ...
  describe('Complete Usage Tracking Flow', () => {
    it('should track usage correctly from start to blocking', async () => {
      ...
      const trackingStarted = await mockUsageRecorder.startTracking(
        tabId,
        'site1'
      );
      ...
      console.log(
        '=== Complete Usage Tracking Flow Test Completed Successfully ==='
      );
