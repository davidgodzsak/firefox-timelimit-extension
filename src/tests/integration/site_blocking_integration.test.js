...
jest.unstable_mockModule('../../background_scripts/site_storage.js', () => ({
  ...
  addDistractingSite: jest
    .fn()
    .mockResolvedValue({ id: '1', urlPattern: 'test.com' }),
  updateDistractingSite: jest
    .fn()
    .mockResolvedValue({ id: '1', urlPattern: 'updated.com' }),
...
jest.unstable_mockModule('../../background_scripts/usage_recorder.js', () => ({
  ...
  getCurrentTrackingInfo: jest
    .fn()
    .mockResolvedValue({ isTracking: false, siteId: null, tabId: null }),
}));
...
describe('Site Blocking Integration', () => {
  beforeEach(async () => {
    ...
    mockRuntimeGetURL.mockImplementation(
    ...
    mockCheckIfUrlIsDistracting.mockImplementation((url) => {
      if (url.includes('example.com')) {
        return {
          isMatch: true,
          siteId: 'distracting-site',
          matchingPattern: 'example.com',
        };
      }
      if (url.includes('timeonly.com')) {
        return {
          isMatch: true,
          siteId: 'time-only-site',
          matchingPattern: 'timeonly.com',
        };
      }
    ...
    mockHandlePotentialRedirect.mockImplementation(async (tabId, url) => {
      ...
      const usage = usageStats[urlCheck.siteId] || {
        timeSpentSeconds: 0,
        opens: 0,
      };
      ...
      const timeExceeded =
        site.dailyLimitSeconds > 0 &&
        usage.timeSpentSeconds >= site.dailyLimitSeconds;
      const opensExceeded =
        site.dailyOpenLimit > 0 && usage.opens >= site.dailyOpenLimit;
    ...
    mockWebNavigationOnBeforeNavigate.addListener.mockImplementation(
      (callback) => {
  ...
  describe('webNavigation.onBeforeNavigate Integration', () => {
    it('should register the navigation event listener', () => {
      expect(
        mockWebNavigationOnBeforeNavigate.addListener
    ...
    it('should handle storage errors gracefully', async () => {
      mockGetDistractingSites.mockRejectedValue(
    ...
    it('should handle tab update errors gracefully', async () => {
      ...
      await expect(
        handleBeforeNavigateCallback(navigationDetails)
